...
class Voice:
    ...
    def on_bored(self):
        return random.choice([
            self._('I\'m bored ...lets reck some n00bs'),
            self._('Let\'s get this bread')])

    def on_motivated(self, reward):
        return self._('This is fricken awesome')

    def on_demotivated(self, reward):
        return self._('its the teams fault not mine')
    ...
    def on_excited(self):
        return random.choice([
            self._('im on a roll'),
            self._('these guys suck at this'),
            self._('is this easy mode?'),
            self._('how long till we get to the big legues')])

    def on_new_peer(self, peer):
        if peer.first_encounter():
            return random.choice([
                self._('Hello {name}! whats cracking').format(name=peer.name())])
        else:
            return random.choice([
    ...
    def on_lost_peer(self, peer):
        return random.choice([
            self._('rest in peperoni {name}').format(name=peer.name()),
            self._('{name} is gone ...').format(name=peer.name())])

    def on_miss(self, who):
        return random.choice([
            self._('Whoops ... {name} is gone.').format(name=who),
            self._('{name} is better than me').format(name=who),
            self._('choked')])
    ...
    def on_napping(self, secs):
        return random.choice([
            self._('taking a piss break for {secs}s ...').format(secs=secs),
            self._('going AFK'),
    ...
    def on_waiting(self, secs):
        return random.choice([
            ...
            '...',
            self._('taking a MountanDew break').format(secs=secs),
            self._('taking a Moster break').format(secs=secs),
            self._('taking a Dorito break').format(secs=secs),
            self._('emptying piss bottles').format(secs=secs),
    ...
    def on_assoc(self, ap):
        ...
        return random.choice([
            ...
            self._('360/d {what}').format(what=what),
            self._('recked {what}').format(what=what),
            self._('finna destroy {what}\'s whole career').format(what=what),
            self._('destroyed {what}').format(what=what),
    ...
    def on_handshakes(self, new_shakes):
        ...
        return self._('Neat-o, we got {num} kill{plural}!').format(num=new_shakes, plural=s)
